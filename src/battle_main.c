...
u8 CreateNPCTrainerPartyFromTrainer(struct Pokemon *party, const struct Trainer *trainer, bool32 firstTrainer, u32 battleTypeFlags)
{
                                                                        ...
                                                                        | BATTLE_TYPE_TRAINER_HILL)))
    {
        ...
        for (i = 0; i < monsCount; i++)
        {
            ...
            if( &party[i] == &gPlayerParty[i] ){
                CreateMon(&party[i], partyData[i].species, partyData[i].lvl, 0, TRUE, personalityValue, otIdType, fixedOtId); //--nox dynamic level caps
            }
            else{
                CreateMon(&party[i], partyData[i].species, GetCurrentLevelCap() - partyData[i].lvl, 0, TRUE, personalityValue, otIdType, fixedOtId); //--nox dynamic level caps
            }
