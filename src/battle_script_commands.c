...
static void Cmd_getexp(void)
{
    ...
    case 1: // calculate experience points to redistribute
        {
            ...
            for (viaSentIn = 0, i = 0; i < PARTY_SIZE; i++)
            {
                ...
                // if (holdEffect == HOLD_EFFECT_EXP_SHARE) -- nox
                //     viaExpShare++;
            }
    ...
    case 2: // set exp value to the poke in expgetter_id and print message
        if (gBattleControllerExecFlags == 0)
        {
            ...
            // if (holdEffect != HOLD_EFFECT_EXP_SHARE && !(gBattleStruct->sentInPokes & 1)) --nox
            if (gExpShareCheck == ((gBattleStruct->sentInPokes & (1 << gBattleStruct->expGetterMonId)) != 0))
            {
                // *(&gBattleStruct->sentInPokes) >>= 1; -- nox
                  ...
                  || GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_LEVEL) == MAX_LEVEL)
            {
                ...
                MonGainEVs(&gPlayerParty[gBattleStruct->expGetterMonId], gBattleMons[gBattlerFainted].species);
                  ...
                  || GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_IS_EGG)){
                    ...
                    gBattleScripting.getexpState = 5;
                    gBattleMoveDamage = 0; // used for exp ??? - nox
            ...
            else
            {
                ...
                {
                    // if (gBattleStruct->sentInPokes & 1) //--nox
                    ...
                    // PrepareStringBattle(STRINGID_PKMNGAINEDEXP, gBattleStruct->expGetterBattlerId); //--Nox
                    if (gBattleStruct->sentInPokes & (1 << gBattleStruct->expGetterMonId))
    ...
    case 5: // looper increment
        ...
        else
        {
            ...
            // else
            //     gBattleScripting.getexpState = 6; // we're done //--Nox
            else{
                s32 totalMon = 0;
                s32 viaSentIn = 0;
                sentIn = gSentPokesToOpponent[(gBattlerFainted & 2) >> 1];
